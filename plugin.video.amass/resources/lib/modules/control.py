...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.amass.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.amass.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.amass.artwork)') else setting('appearance.alt')
    if appearance == "Amass": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.amass.artwork)')
