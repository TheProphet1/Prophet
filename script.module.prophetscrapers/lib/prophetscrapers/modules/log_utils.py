...
def log(msg, level=LOGNOTICE):
    debug_enabled = control.addon('plugin.video.prophet').getSetting('addon_debug')
    debug_log = control.addon('plugin.video.prophet').getSetting('debug.location')

    print(DEBUGPREFIX + ' Debug Enabled?: ' + str(debug_enabled))
    print(DEBUGPREFIX + ' Debug Log?: ' + str(debug_log))

    if not control.addon('plugin.video.prophet').getSetting('addon_debug') == 'true':
        return
    ...
    try:
        ...
        if not control.addon('plugin.video.prophet').getSetting('debug.location') == '0':
