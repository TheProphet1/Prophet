...
DEBUGPREFIX = '[ ProphetScrapers DEBUG ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'prophet.log')
debug_enabled = control.addon('plugin.video.prophet').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('ProphetScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
