...
addonObject = addon('script.module.prophetscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from prophetscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.prophet.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.prophet.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.prophet.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.prophet.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.prophet.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.prophet.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
