...
from prophetscrapers.modules import cleandate
...
V2_API_KEY = control.addon('plugin.video.prophet').getSetting('trakt.client_id')
CLIENT_SECRET = control.addon('plugin.video.prophet').getSetting('trakt.client_secret')
REDIRECT_URI = 'urn:ietf:wg:oauth:2.0:oob'

if V2_API_KEY == "" or CLIENT_SECRET == "":
    V2_API_KEY = base64.b64decode("NmVhMDc1YjFhY2NhMmZiYmM1NTA1MTdhNzA5ZDU1MzAyMDcxNWMyZWRmOTA2MDI3ZDZlNzgzMjMwYThlYzZiZA==")
    CLIENT_SECRET = base64.b64decode("ZWUxY2ZmMDk1NTZlYzc4NzZiMThkODJjYmYyOTI5MmVkMTM5MTllZWZmNWM4MDBkM2UwNWNlYTIwNjc2NGIyNw==")
...
def __getTrakt(url, post=None):
    try:
        ...
        if getTraktCredentialsInfo():
            headers.update({'Authorization': 'Bearer %s' % control.addon('plugin.video.prophet').getSetting('trakt.token')})
        ...
        opost = {'client_id': V2_API_KEY, 'client_secret': CLIENT_SECRET, 'redirect_uri': REDIRECT_URI, 'grant_type': 'refresh_token', 'refresh_token': control.addon('plugin.video.prophet').getSetting('trakt.refresh')}
        ...
        control.addon('plugin.video.prophet').setSetting(id='trakt.token', value=token)
        control.addon('plugin.video.prophet').setSetting(id='trakt.refresh', value=refresh)
    ...
    try:
        if getTraktCredentialsInfo() == True:
            if control.yesnoDialog(control.lang(32511) + '[CR]' + control.lang(32512), heading='Trakt'):
                control.addon('plugin.video.prophet').setSetting(id='trakt.user', value='')
                control.addon('plugin.video.prophet').setSetting(id='trakt.authed', value='')
                control.addon('plugin.video.prophet').setSetting(id='trakt.token', value='')
                control.addon('plugin.video.prophet').setSetting(id='trakt.refresh', value='')
            raise Exception()
        ...
        print('info - ' + token)
        control.addon('plugin.video.prophet').setSetting(id='trakt.user', value=user)
        control.addon('plugin.video.prophet').setSetting(id='trakt.authed', value=authed)
        control.addon('plugin.video.prophet').setSetting(id='trakt.token', value=token)
        control.addon('plugin.video.prophet').setSetting(id='trakt.refresh', value=refresh)
        raise Exception()
    except:
...
def getTraktCredentialsInfo():
    user = control.addon('plugin.video.prophet').getSetting('trakt.user').strip()
    token = control.addon('plugin.video.prophet').getSetting('trakt.token')
    refresh = control.addon('plugin.video.prophet').getSetting('trakt.refresh')
    if (user == '' or token == '' or refresh == ''): return False
    return True


def getTraktIndicatorsInfo():
    indicators = control.addon('plugin.video.prophet').getSetting('indicators') if getTraktCredentialsInfo() == False else control.addon('plugin.video.prophet').getSetting('indicators.alt')
...
def cachesyncMovies(timeout=0):
    indicators = cache.get(syncMovies, timeout, control.addon('plugin.video.prophet').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncMovies():
    timeout = cache.timeout(syncMovies, control.addon('plugin.video.prophet').getSetting('trakt.user').strip())
    return timeout
...
def cachesyncTVShows(timeout=0):
    indicators = cache.get(syncTVShows, timeout, control.addon('plugin.video.prophet').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncTVShows():
    timeout = cache.timeout(syncTVShows, control.addon('plugin.video.prophet').getSetting('trakt.user').strip())
    return timeout
