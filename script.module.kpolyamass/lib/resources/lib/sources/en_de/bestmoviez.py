...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            #     links = ['%s-%s-2160p' % (search_title, self.hdlr), '%s-%s-1080p' % (search_title, self.hdlr), '%s-%s-720p' % (search_title, self.hdlr), \
            #              '%s-%s' % (search_title, self.hdlr), '%s-2160p' % (search_title), '%s-1080p' % (search_title), '%s-720p' % (search_title), \
            #              '%s' % (search_title), '%s-%s-3d' % (search_title, self.hdlr), '%s-3d' % (search_title)]
            # else:
            #     search_title = cleantitle.geturl(title)
            #     links = ['%s-%s-2160p' % (search_title, self.hdlr), '%s-%s-1080p' % (search_title, self.hdlr), '%s-%s-720p' % (search_title, self.hdlr), \
            #              '%s-%s' % (search_title, self.hdlr)]
            threads = []
            for i in links:
